...
func InitializeCreatePaymentScenario(ctx *godog.ScenarioContext) {
    ...
    ctx.Step(`^the dinopay gateway sends the following payment to the payments endpoint:$`, theDinopayGatewaySendsTheFollowingPayment)
...
func theDinopayGatewaySendsTheFollowingPayment(ctx context.Context, rawPayment *godog.DocString) (context.Context, error) {
    res, err := createPayment(ctx, rawPayment.Content, privateApiHttpServerPort)
    if err != nil {
        return ctx, err
    }
