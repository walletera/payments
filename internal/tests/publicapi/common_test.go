...
import (
    ...
    "github.com/walletera/payments/internal/tests/publicapi/httpauth"
    ...
    go func() {
        app, err := app.NewApp(
            ...
            app.WithPublicAPIConfig(app.PublicAPIConfig{
                ...
                AuthServiceBase64PubKey: string(base64Key),
            }),
...
func createPayment(ctx context.Context, paymentJson string, port int) (publicapi.PostPaymentRes, error) {
    ...
    requestCtx, cancelCtx := context.WithTimeout(ctx, 5*time.Second)
    defer cancelCtx()
    res, err := paymentsClient.PostPayment(requestCtx, &paymentCreationRequest, publicapi.PostPaymentParams{})
    if err != nil {
        return nil, err
    }
    return res, nil
